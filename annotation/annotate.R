...
source("annotation/tool_interfaces/SCINA_interface.R")
...
annotate <- function(expr_data, ref_data, annot_type, annot_tools = "*",
  ...
  if (!is(expr_data, "Matrix") & !is(expr_data, "matrix")) {
    expr_data <- get_assay_data(expr_data, args$expr_type)
  }

  if (!is(ref_data, "Matrix") & !is(ref_data, "matrix")) {
  ...
  lapply(
    ...
    function(x) {
      run_full(x, expr_data, ref_data, convert_to,
